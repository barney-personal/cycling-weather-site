import{m as S}from"./header-pbtJ5vNN.js";import{g as R,f as k,h as q}from"./destination-meta-xTBeTcjo.js";import{d as H,D as N,b as F}from"./qualify-CS5O-wsU.js";import{l as P}from"./data-DO2zHL4L.js";const I="modulepreload",A=function(t,e){return new URL(t,e).href},x={},B=function(e,n,l){let o=Promise.resolve();if(n&&n.length>0){const r=document.getElementsByTagName("link"),d=document.querySelector("meta[property=csp-nonce]"),u=d?.nonce||d?.getAttribute("nonce");o=Promise.allSettled(n.map(a=>{if(a=A(a,l),a in x)return;x[a]=!0;const p=a.endsWith(".css"),v=p?'[rel="stylesheet"]':"";if(!!l)for(let h=r.length-1;h>=0;h--){const y=r[h];if(y.href===a&&(!p||y.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${a}"]${v}`))return;const c=document.createElement("link");if(c.rel=p?"stylesheet":I,p||(c.as="script"),c.crossOrigin="",c.href=a,u&&c.setAttribute("nonce",u),document.head.appendChild(c),p)return new Promise((h,y)=>{c.addEventListener("load",h),c.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${a}`)))})}))}function s(r){const d=new Event("vite:preloadError",{cancelable:!0});if(d.payload=r,window.dispatchEvent(d),!d.defaultPrevented)throw r}return o.then(r=>{for(const d of r||[])d.status==="rejected"&&s(d.reason);return e().catch(s)})};function i(t){return t.replace(/[&<>"']/g,e=>e==="&"?"&amp;":e==="<"?"&lt;":e===">"?"&gt;":e==='"'?"&quot;":"&#39;")}function b(t){if(!t)return"";const e=new Date(`${t}T00:00:00Z`);return Number.isNaN(e.getTime())?t:e.toLocaleDateString(void 0,{month:"short",day:"numeric",timeZone:"UTC"})}function D(t){const e=new Date(`${t}T00:00:00Z`);return Number.isNaN(e.getTime())?t:e.toLocaleDateString(void 0,{weekday:"short",month:"short",day:"numeric",timeZone:"UTC"})}const U={0:"Clear",1:"Mostly clear",2:"Partly cloudy",3:"Overcast",45:"Fog",48:"Rime fog",51:"Light drizzle",53:"Drizzle",55:"Heavy drizzle",61:"Light rain",63:"Rain",65:"Heavy rain",71:"Light snow",73:"Snow",75:"Heavy snow",77:"Snow grains",80:"Rain showers",81:"Heavy showers",82:"Violent showers",95:"Thunderstorm",96:"Thunder + hail",99:"Heavy thunder"},O={0:"☀",1:"🌤",2:"⛅",3:"☁",45:"🌫",48:"🌫",51:"🌦",53:"🌦",55:"🌧",61:"🌧",63:"🌧",65:"🌧",71:"❄",73:"❄",75:"❄",77:"❄",80:"🌧",81:"🌧",82:"⛈",95:"⛈",96:"⛈",99:"⛈"};function W(t){return U[t]??`Code ${t}`}function Z(t){return O[t]??"·"}function z(t,e){return e?"go":t>=5?"edge":"no-go"}function j(t,e){return t.latest?t.latest.results.find(n=>n.slug===e)??null:null}function G(t,e){return t.filter(n=>n.name===e).slice().sort((n,l)=>n.date.localeCompare(l.date)).slice(-30)}function K(t){return t.length===0?'<p class="dest-routes-empty">Curated route notes coming soon.</p>':`<ul class="dest-routes">${t.map(e=>`<li class="dest-route">
      ...
      </li>`).join("")}</ul>`}function Q(t){return t.length===0?"":`<ul class="dest-tags" aria-label="Ride types">${t.map(e=>`<li class="dest-tag">${i(e)}</li>`).join("")}</ul>`}function V(t,e,n){return`<li class="dest-day${e?" is-qualify":""}" aria-label="${i(`Day ${n+1}: ${D(t.date)}`)}">
  ...
  </dl>`}function ee(t,e){t.innerHTML=`
    ...
    </section>`}function te(t){const{mount:e,data:n,slug:l}=t,o=t.thresholds??N,s=j(n,l);if(!s){ee(e,l);return}const r=R(l),d=q(l),u=s.daily.map(m=>H(m,o)),a=F(u),p=s.daily.reduce((m,g)=>m+(g.precip_sum<=o.rainMax&&g.precip_prob_max<o.probMax?1:0),0),v=["lake-district","peak-district","yorkshire-dales","south-wales","london-surrey-hills"].includes(l),$=a.length>=7&&(!v||s.median_temp>20),c=z(a.length,$),h=n.latest?.forecast_date??null,y=G(n.actuals_timeline,s.name),M=s.region?`<span class="dest-region">${i(s.region)}</span>`:"",T=c==="go"?"GO":c==="edge"?"EDGE":"NO-GO",L=a.startIdx!==null&&a.endIdx!==null?a.startIdx===a.endIdx?b(s.daily[a.startIdx]?.date??null):`${b(s.daily[a.startIdx]?.date??null)} → ${b(s.daily[a.endIdx]?.date??null)}`:"—",C=!$&&s.blocker?`<p class="dest-blocker">${i(s.blocker)}</p>`:"",E=d?"":'<p class="dest-fallback">Curated guide notes are still being written for this destination.</p>';e.innerHTML=`
    <article class="dest-page" data-slug="${i(l)}" data-verdict="${c}">
      ...
      <section class="dest-section" aria-labelledby="dest-forecast-title">
        ...
        <ul class="dest-day-grid">
          ${s.daily.map((m,g)=>V(m,u[g]===!0,g)).join("")}
        </ul>
      </section>
    ...
    </article>`;const f=e.querySelector("#dest-polar-mount");f&&B(async()=>{const{mountPolar:m}=await import("./polar-CX9LcEsr.js");return{mountPolar:m}},[],import.meta.url).then(({mountPolar:m})=>{m({mount:f,days:s.daily,qualifies:u,label:s.name}),f.removeAttribute("aria-busy")}).catch(()=>{f.innerHTML='<p class="polar-empty">Polar chart unavailable.</p>',f.removeAttribute("aria-busy")})}S({mount:"#site-header",active:"forward"});const se=new URLSearchParams(window.location.search),w=(se.get("slug")??"").toLowerCase(),_=document.getElementById("dest-mount");_&&(_.dataset.slug=w,P().then(t=>{te({mount:_,data:t,slug:w});const e=t.latest?.results.find(n=>n.slug===w);e&&(document.title=`${e.name} · Cycling Weather`)}));
